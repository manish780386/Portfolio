...
function Typewriter({ texts, speed = 65 }) {
  ...
  useEffect(() => {
    ...
    if (!deleting && charIdx < current.length) {
      ...
      t = setTimeout(() => { setCharIdx((c) => c - 1); setDisplayed(current.slice(0, charIdx - 1)); }, speed / 2.5);
    } else {
      setDeleting(false);
  ...
  return (
    <span className="font-mono-label text-[#34d399]">
      {displayed}
      <motion.span animate={{ opacity: [1, 0, 1] }} transition={{ repeat: Infinity, duration: 0.9 }}>_</motion.span>
    </span>
  );
}
...
export default function Hero() {
  ...
  return (
    <section id="home" className="relative min-h-screen flex items-center overflow-hidden pt-20 md:pt-0">
      <motion.div style={{ opacity: fade }} className="relative z-10 w-full max-w-6xl mx-auto px-6 md:px-10 grid lg:grid-cols-[1.15fr_0.85fr] gap-14 items-center">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }} className="flex flex-wrap items-center gap-2">
            ...
            <span className="font-mono-label text-[10px] uppercase tracking-widest text-[#4b5768] px-2.5 py-1 rounded-full border border-white/10">
              Full-Stack × Security
            </span>
          </motion.div>
        ...
        {/* RIGHT — ACCESS CARD (signature element) */}
        <motion.div
        ...
        >
          <div className="relative w-[280px]">
            {/* orbiting ring — signature glow behind the card */}
            <motion.div
              animate={{ rotate: 360 }}
              transition={{ repeat: Infinity, duration: 10, ease: "linear" }}
              className="absolute -inset-3 rounded-[22px] opacity-70 pointer-events-none"
              style={{ background: "conic-gradient(from 0deg, transparent 0deg, #34d399 80deg, transparent 160deg, #60a5fa 260deg, transparent 340deg)" }}
            />
            <div className="absolute -inset-3 rounded-[22px] bg-[#060a11]" />

            <div className="relative rounded-2xl overflow-hidden border border-white/10 bg-[#0d131d]"
  ...
  decoding="async" />
                ...
                <div className="w-full space-y-2">
                  <div className="flex justify-between text-[11px]">
                    <span className="text-[#4b5768]">Clearance</span>
                    <span className="text-[#34d399] font-mono-label">Full-Stack + Sec</span>
                  </div>
                  <div className="flex justify-between text-[11px]">
                ...
                <div className="flex gap-2 mt-5 w-full">
                  ...
                  <a href="https://leetcode.com/u/dangemanish/" target="_blank" rel="noopener noreferrer"
                    className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg bg-white/[0.04] border border-white/[0.08] text-[#7c8aa0] hover:text-white text-xs transition">
      ...
      <motion.div className="absolute bottom-8 left-1/2 -translate-x-1/2 hidden md:flex flex-col items-center gap-2 z-10"
